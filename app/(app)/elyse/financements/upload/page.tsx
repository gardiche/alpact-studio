...
import { fileToBase64 } from '@/lib/financements/geminiService';
...
export default function UploadPage() {
  ...
  const handleProcessFile = async (selectedFile: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/financements/parse-deck", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ base64Data: base64, mimeType: selectedFile.type }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "Erreur serveur" }));
        throw new Error(err.error ?? "Analyse échouée");
      }
      const { projectData: extractedProject, questionnaireData: extractedQuestionnaire } = await res.json();
