...
import type { HubMetrics } from "@/lib/hub/hubRepository";

export function PulseCards({ metrics }: { metrics: HubMetrics }) {
  const { mrr, mrrTrend, runway, runwayStatus, priorite, alertes } = metrics;
  ...
  return (
    <div className="grid grid-cols-4 gap-4 mb-8">
      ...
      <MetricCard
        label="Runway"
        value={runway}
        subLabel={runwayStatus === "warning" ? "⚠️ Sous le seuil recommandé" : runwayStatus === "critical" ? "🔴 Critique" : "Bonne santé"}
        icon={<Clock size={16} />}
      />
      <MetricCard
